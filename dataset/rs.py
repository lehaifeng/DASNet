...
class Dataset(Dataset):
    ...
    def get_img_label_path_pairs(self):
        ...
        if self.flag =='train':
            for idx , did in enumerate(open(self.img_txt_path)):
                ...
                extract_name = image1_name[image1_name.rindex('/') +1: image1_name.rindex('.')] # rindex() 返回子字符串 str 在字符串中最后出现的位置,默认是从右到左
                ...
                img_label_pair_list.setdefault(idx, [img1_file,img2_file,lbl_file,image2_name]) # 向字典中添加新的key-value对
        if self.flag == 'val':
            self.label_ext = '.png'
            for idx , did in enumerate(open(self.img_txt_path)):
                try:
    ...
    def data_transform(self, img1,img2,lbl):
        img1 = img1[:, :, ::-1]  # (h, w, RGB) -> (h, w, BGR)
        img1 = img1.astype(np.float64)
        img1 -= cfg.T0_MEAN_VALUE
        img1 = img1.transpose(2, 0, 1) # numpy : (h,w,c) -> pytorch : (c,h,w) 
        img1 = torch.from_numpy(img1).float()
        
        img2 = img2[:, :, ::-1]  # RGB -> BGR
        img2 = img2.astype(np.float64)
        img2 -= cfg.T1_MEAN_VALUE
        img2 = img2.transpose(2, 0, 1) # numpy : (h,w,c) -> pytorch : (c,h,w) 
